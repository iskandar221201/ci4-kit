...
class UserController extends BaseApiController
{
    ...
    private function sanitizeUser(?object $user): ?array
    {
        ...
        // Shield User entity exposes password/password_hash as private fields,
        // so they won't appear in standard serialization. We cast to array via
        // the entity's own toArray() or JSON-encode trick and then remove
        // any remaining sensitive keys defensively.
        $data = json_decode(json_encode($user), true) ?? [];

        // Defensive removal of any sensitive keys that should never be exposed
        foreach (['password', 'password_hash', 'secret', 'secret2', 'last_ip', 'force_pass_reset'] as $key) {
            unset($data[$key]);
        }
